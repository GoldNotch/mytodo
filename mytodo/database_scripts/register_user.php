<?php 
	...
	function registrate($login, $password)
	{
		...
		if (!$is_exists){
			...
			$final_login = '\''.$conn->escape_string($login).'\''.'@'.'\''.$conn->escape_string(servername).'\'';
			$sql = 'CREATE USER '.$final_login.' IDENTIFIED BY \''.$conn->escape_string($password).'\'';
			echo $sql;
			if (!$conn->query($sql))
				die($conn->error);

			//------------ Даем привилегии пользователю ---------------
			$sql = 'GRANT SELECT, INSERT, DELETE, UPDATE ON mytaskmanagerdatabase.tasks TO '.$final_login;
			if (!$conn->query($sql))
			 	die($conn->error);
		}
